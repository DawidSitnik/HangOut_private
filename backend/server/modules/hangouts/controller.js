...
export const createHangout = async (req, res) => {
  ...
  const { id, activity, description, coordinates, timeFrom, timeTo, radius, maxPeople } = req.body;
  ...
  let newActivity = activityLowercase.charAt(0).toUpperCase() + activityLowercase.slice(1)
  let newTimeFrom = timeFrom.substr(5).replace(/-/g, ".").replace(/T/g, " ")
  let newTimeTo = timeTo.substr(5).replace(/-/g, ".").replace(/T/g, " ")
...
export const getHangoutsAroundUser = async (req, res) => {
  ...
  for( let hangout of hangouts){
    const distance = getDistanceFromLatLonInKm(user.coordinates.latitude, user.coordinates.longitude, hangout.coordinates.latitude, hangout.coordinates.longitude)
    ...
    if(hangout.users.length <= hangout.maxPeople){
      if (distance < 10000) {
        ...
        if (!isUserInHangout) hangoutsAroundUser.push({hangout: hangout, distance: distance})
  ...
  try {
    ...
    return res.status(e.status).json({ error: true, message: 'Error with getting hangouts' });
  }
};
...
export const findCreatedHangouts = async (req, res) => {
  ...
  for(let hangoutId of hangoutsIds){
    ...
    let distance = getDistanceFromLatLonInKm(hangout.coordinates.latitude, hangout.coordinates.longitude,
...
export const findHangouts = async (req, res) => {
  ...
  for (let hangout of allHangouts){
        if(hangout.users.length <= hangout.maxPeople){
          let distance = getDistanceFromLatLonInKm(user.coordinates.latitude, user.coordinates.longitude,
          hangout.coordinates.latitude, hangout.coordinates.longitude)
          if(distance < (howFar + hangout.radius)/1000){
            ...
            if(!isUserInHangout) foundHangouts.push({hangout, distance: distance})
            isUserInHangout = false;
          }
        }
  }
...
export const deleteHangout = async (req, res) => {
    const { userId, hangoutId } = req.body;
    await User.findByIdAndUpdate(userId, { $pull: { createdHangouts: hangoutId } })
    let hangout = await Hangout.findById(hangoutId)
    ...
    try {
      ...
      return res.status(e.status).json({ error: true, message: 'Error with deleting hangout' });
    }
};
...
export const getPeopleInHangout  = async (req, res) => {
  ...
  for (let personId of people){
    ...
    const person = await User.findById(personId, { _id: 1, name: 1, description:1, profilePicture:1 })
  ...
  for(let hangout of hangouts){
    ...
    if(year > hangoutYear)
      {await Hangout.deleteOne({"_id":hangout._id}); }
          ...
          {await Hangout.deleteOne({"_id":hangout._id}); }
            else if(day > hangoutDay && month == hangoutMonth && year == hangoutYear)
              {await Hangout.deleteOne({"_id":hangout._id}); }
                else if(hour > hangoutHour && day == hangoutDay && month == hangoutMonth && year == hangoutYear)
                  {await Hangout.deleteOne({"_id":hangout._id}); }
                    else if(minute > hangoutMinute && hour == hangoutHour && day == hangoutDay && month == hangoutMonth && year == hangoutYear)
                      {await Hangout.deleteOne({"_id":hangout._id}); }
